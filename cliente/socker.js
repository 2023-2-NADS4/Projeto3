  ...
  messages.forEach((message) => {
    ...
    if (message.receiver.id == "me") {
      ...
      if (last == "me") {
        ...
        div.innerHTML = `
        <p style="width: 100%; text-align: start;">${message.sender.name} </p>
        <p class="message others-message">${message.text}</p>`;
      }
      last = "me";
    } else {
      if (last == "mine") {
        ...
        <p class="message before-mine my-message" id="${message.receiver.id}:${SENT_MESSAGE_COUNT}">${message.text}</p>
        
        `;
      } else {
        div.innerHTML = `
        <p style="width: 100%; text-align: end;">Voce</p>
        <p class="message my-message" id="${message.receiver.id}:${SENT_MESSAGE_COUNT}">${message.text}</p>
        `;
      }
      last = "mine";
    }
...
function appendContactsToUI(contacts) {
  ...
  contacts.forEach((contact) => {
    ...
    <div>
                ...
                <div style="flex: 2; text-align: left; padding: 0; width: 80%;">
                    <p style="
                    width: 80%;
                    text-overflow: ellipsis;
                      overflow: hidden;
                       white-space: nowrap; 
                        display: inherit;
                    font-size: 18px; font-weight: 600; color: gray;">${
                      contact.name
                    }</p>
                    <p id="contact-${contact.id}-last-message" style="
                    width: 80%;
                    text-overflow: ellipsis;
                      overflow: hidden;
                       white-space: nowrap; 
                    ...
                    color: gray;" class="fsz-09">${contact.last_message}</p>
        
                    <div style="margin-top: 0; gap: 8px;" class="f">
                        
                
                        <p style="width: 18px; height: 18px; border-radius: 50%; background: var(--main-color); color: white; font-weight: 600; display: none; text-align: center;" class="f f-cntr fsz-08" id="contact-${
                          contact.id
                        }-not-read-count">${contact.not_read_count || ""}</p>
                    </div>
                </div>
    `;
    ...
    if (contact.not_read_count) {
      document.getElementById(
        `contact-${contact.id}-not-read-count`
      ).style.display = "unset";
    }
  });
}
...
socket.addEventListener("open", (event) => {
  ...
  document.getElementById("chat-info-main")?.remove();
  setTimeout(() => {
